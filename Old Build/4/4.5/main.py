...
async def invia_messaggio(messaggio):
    ...
    await bot.send_message(chat_id=config.chat_id, text=messaggio)
...
def controlla_connessione():
    ...
    if len(connessioni_persone) == len(config.indirizzi_ping):
    ...
    async def monitoraggio():
        ...
        while True:
            if not modalita_manutenzione:
                ...
                connessione_attuale, nome_dispositivo = controlla_connessione()
                if not connessione_attuale:
                    print(f"La connessione Ethernet è interrotta tramite {nome_dispositivo}. Invio del messaggio di avviso.")
                    await invia_messaggio(f"⚠️ Avviso: la connessione Ethernet è persa tramite {nome_dispositivo}.")
                    # Scrivi l'evento nel file di log
                    scrivi_log("Connessione interrotta", nome_dispositivo)
                    # Esegui il controllo ogni 150 secondi
                    for _ in range(3):
                        await asyncio.sleep(150)
                        connessione_attuale, nome_dispositivo = controlla_connessione()
                        if connessione_attuale:
                            print(f"La connessione Ethernet è ripristinata tramite {nome_dispositivo}. Invio del messaggio di ripristino.")
                            await invia_messaggio(f"✅ La connessione Ethernet è ripristinata tramite {nome_dispositivo}.")
                            scrivi_log("Connessione ripristinata", nome_dispositivo)
                            break
                        else:
